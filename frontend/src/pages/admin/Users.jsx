...
function PackageBadge({ name, daysLeft }) {
  ...
  return (
    <div className="au-pkg-wrap">
      <span className={`au-pkg-badge ${expired ? 'au-pkg-expired' : warning ? 'au-pkg-warning' : 'au-pkg-active'}`}>
        {name}
      </span>
      {!expired && daysLeft !== null && (
        <span className="au-pkg-days">{daysLeft} ngày</span>
      )}
      {expired && <span className="au-pkg-days au-pkg-days-expired">Hết hạn</span>}
    </div>
  );
}
...
export default function AdminUsers({ user, onLogout }) {
  ...
  return (
    <AdminLayout user={user} onLogout={onLogout} title="Quản lý người dùng" subtitle="Xem và quản lý tài khoản người dùng trong hệ thống">
      {/* STATS */}
            ...
            <tbody>
              ...
              {!loading && users.map((u, i) => (
                <tr key={u.id}>
                  ...
                  <td>
                    {u.role === 'employer'
                      ? <PackageBadge name={u.packageName} daysLeft={u.packageDaysLeft} />
                      : '—'}
                  </td>
                  ...
                  <td>
                    <div className="adm-action-btns">
                      ...
                      <button
                        className={u.active ? 'au-btn-lock' : 'au-btn-unlock'}
              ...
              <div className="ar-modal-rows">
                ...
                {detail.role === 'employer' && (
                  <div className="ar-modal-row">
                    ...
                    <PackageBadge name={detail.packageName} daysLeft={detail.packageDaysLeft} />
                  </div>
                )}
                <div className="ar-modal-row">
